"""Task creation and inline (HTMX) editing: notes, due date, status."""
...
from Onboarding.models import StatusEnum, Task
from Onboarding.utils.dates import parse_due_date_strict
...
def update_notes(task_id):
    ...
    t.notes = raw_notes.strip()
    db.session.commit()

    if request.headers.get("HX-Request"):
        return render_template("_task_notes_display.html", t=t, user=user)
    return redirect(
        url_for("weeks.week_detail", week_id=t.week_id, as_user=user.email)
    )
...
def update_due_date(task_id: int):
    ...
    parsed = parse_due_date_strict(raw)
